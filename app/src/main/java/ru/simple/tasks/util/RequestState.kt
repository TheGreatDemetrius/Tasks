...
sealed class RequestState<out T> {//модификатор out определяет вариантность обобщения
    ...
    data class Success<T>(val data: T): RequestState<T>()//наследуемся от RequestState, который возвращает все данные
    data class Error(val error: Throwable): RequestState<Nothing>()//наследуемся от RequestState, который ничего не возвращает
}